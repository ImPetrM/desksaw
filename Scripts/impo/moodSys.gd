...
func moodCheck():
		...
		mood = gbData.data.saw[petId].mood
...
func calcmood(total: float):
	...
	if mas.ragdolled and not mas.isSleeping:
		...
		total += 1.5
	total -= clamp(((1.0 - (gbData.data.saw[petId]["hunger"] * 0.01)) * 1), 0.0, 1)
	#trust
	total -= clamp(((1.0 - (gbData.data.saw[petId]["tired"] * 0.01)) * 1), 0.0, 1)
	print(str(clamp(((1.0 - (gbData.data.saw[petId]["tired"] * 0.01)) * 1), 0.0, 1)) + " 123123")
	if gbData.data.saw[petId]["hunger"] >= 85.0:
		total += 0.25
